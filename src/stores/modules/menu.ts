...
/** 请求后端（开发下 Mock 开启时由 vite-plugin-mock 拦截） */
const fetchMenuFromServer = async (): Promise<MenuOption[]> => {
  ...
  return menuData.map((item: any) => ({
    ...
    children: item.children?.map((child: any) => ({
      ...
      icon: child.meta?.icon
        ? renderIcon(iconMap[child.meta.icon] || DashboardOutlined)
        : undefined,
    })),
  }));
};
  ...
  actions: {
    async initMenu() {
      ...
      try {
        ...
        this.menuOptions = [...routeMenu, ...serverMenu];
      } catch (error) {
    ...
    /** 仅使用接口返回的菜单树（开发下 Mock 开启时走 vite-plugin-mock） */
    async reloadMenuFromServer() {
      this.isLoading = true;
      try {
        const serverMenu = await fetchMenuFromServer();
        this.menuOptions = serverMenu;
      } catch (error) {
        console.error('获取菜单失败:', error);
      } finally {
        this.isLoading = false;
